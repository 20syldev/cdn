...
function parseDigits(digits, from) {
    const radix = BigInt(from);
    let parsed = 0n;
    for (const char of digits) {
        const digit = DIGITS.indexOf(char);
        if (digit === -1 || digit >= from)
            throw new Error(`Invalid value for base ${from}`);
 ...
 */
export default function base(value, from = 10, to = 16) {
    if (!value)
        throw new Error('Please provide a value (?value={value})');
