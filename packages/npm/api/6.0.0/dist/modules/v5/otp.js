...
function generateHotp(secret, counter, algorithm, digits) {
 ...
 */
export default function otp(action, options = {}) {
    ...
    if (!VALID_PERIODS.has(period))
        throw new Error('Period must be 15, 30, or 60');
    if (action === 'secret') {
    ...
    if (!options.secret)
        throw new Error('Secret is required for generate/verify');
    ...
    if (action === 'generate') {
        const counter = options.counter !== undefined ? BigInt(options.counter) : BigInt(Math.floor(Date.now() / 1000 / period));
    ...
    // action === 'verify'
    if (!options.code)
        throw new Error('Code is required for verify');
    const currentCounter = options.counter !== undefined ? BigInt(options.counter) : BigInt(Math.floor(Date.now() / 1000 / period));
    for (let drift = 0; drift <= 1; drift++) {
        const counter = currentCounter - BigInt(drift);
        if (counter < 0n)
            continue;
        const code = generateHotp(secretBuf, counter, algorithm, digits);
        if (codesEqual(code, options.code))
            return { valid: true, drift: drift === 0 ? 0 : -drift };
    }
