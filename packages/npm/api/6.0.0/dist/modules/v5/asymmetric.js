import { constants as cryptoConstants, createPublicKey, generateKeyPairSync, privateDecrypt, publicEncrypt, } from 'crypto';
 ...
 */
export default function asymmetric(action, options = {}) {
    ...
    if (!algo)
        throw new Error(`Unsupported algorithm. Use one of: ${Object.keys(ALGORITHMS).join(', ')}`);
    if (action === 'keygen') {
    ...
    if (action === 'encrypt') {
        ...
        if (!options.publicKey.startsWith('-----BEGIN PUBLIC KEY-----'))
        ...
        try {
            const encrypted = publicEncrypt({ key: keyObject, oaepHash: algo.oaepHash, padding: cryptoConstants.RSA_PKCS1_OAEP_PADDING }, Buffer.from(options.text, 'utf8'));
    ...
    try {
        const decrypted = privateDecrypt({ key: options.privateKey, oaepHash: algo.oaepHash, padding: cryptoConstants.RSA_PKCS1_OAEP_PADDING }, Buffer.from(options.text, 'base64'));
