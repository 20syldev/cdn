...
const muted = () => `rgb(${Math.floor(Math.random() * 180)}, ${Math.floor(Math.random() * 180)}, ${Math.floor(Math.random() * 180)})`;
/**
...
function hardened(options) {
    ...
    for (let i = 0; i < Math.floor(text.length * 1.5); i++) {
        ...
        ctx.fillText(CHARSET[Math.floor(Math.random() * CHARSET.length)], Math.random() * width, Math.random() * height);
    }
    ...
    for (let i = 0; i < 2 + Math.floor(Math.random() * 3); i++) {
        ...
        ctx.beginPath();
        const band = () => height * 0.3 + Math.random() * height * 0.4;
        ctx.moveTo(0, band());
 ...
 * @throws Error if any option is out of the accepted range
 */
export default function captchaChallenge(options) {
 ...
 */
export function verifyCaptcha(token, answer, storage) {
