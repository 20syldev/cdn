    ...
    switch (node.kind) {
        ...
        case 'call': {
            ...
            switch (node.name) {
                ...
                case 'min':
                    if (args.length !== 2)
                        throw new Error('Function min() requires exactly 2 arguments');
                    return Math.min(args[0], args[1]);
                case 'max':
                    if (args.length !== 2)
                        throw new Error('Function max() requires exactly 2 arguments');
...
export default function evaluate(expr, precision) {
    ...
    const prec = Math.min(15, Math.max(0, precision !== undefined && Number.isFinite(precision) ? Math.floor(precision) : 10));
