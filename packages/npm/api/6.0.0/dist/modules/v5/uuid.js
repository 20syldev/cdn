 ...
 */
export default function uuid(input, count) {
    if (input) {
        if (!UUID_REGEX.test(input))
            return { uuid: input, version: null, variant: null, valid: false };
    ...
    const total = count ?? 1;
    if (total > 1)
        return { uuids: Array.from({ length: total }, () => randomUUID()), count: total };
