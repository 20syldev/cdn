...
function parseDisplaySize(v, name) {
    if (v === undefined || v === null)
        return null;
    const n = Number(v);
    if (!isFinite(n) || n <= 0)
        throw new Error(`Field '${name}' must be a positive number`);
    return Math.floor(n);
}
function getColors(colors, n) {
    const result = [];
...
function svgOpen(bg, renderW, renderH, displayW, displayH) {
...
function validateBarLine(data) {
    ...
    if (d['labels'].length > MAX_CHART_LABELS)
        throw new Error(`Cannot exceed ${MAX_CHART_LABELS} labels`);
    if (!Array.isArray(d['datasets']) || d['datasets'].length === 0)
        throw new Error("Field 'data.datasets' must be a non-empty array");
    if (d['datasets'].length > MAX_CHART_DATASETS)
        throw new Error(`Cannot exceed ${MAX_CHART_DATASETS} datasets`);
    for (const ds of d['datasets']) {
        ...
        for (const v of dataset['values']) {
            if (typeof v !== 'number' || !isFinite(v))
                throw new Error('Dataset values must be finite numbers');
        }
    }
    return {
...
function validatePieDonut(data) {
    if (!data || typeof data !== 'object')
        throw new Error("Field 'data' is required");
    const d = data;
    if (!Array.isArray(d['labels']) || d['labels'].length === 0)
    ...
    if (total <= 0)
        throw new Error('At least one pie/donut value must be positive');
    return {
        labels: d['labels'].map(String),
        values: d['values'],
    };
...
function toY(value, min, max, chartH) {
...
export function line(data, options) {
    ...
    const xPos = (i) => MARGIN.left + (d.labels.length > 1 ? i * xStep : chartW / 2);
    let svg = svgOpen(bg, renderW, renderH, displayW, displayH);
    if (title)
        svg += `  <text x="${renderW / 2}" y="${MARGIN.top}" text-anchor="middle" font-family="sans-serif" font-size="14" font-weight="bold" fill="#333">${escapeXml(title)}</text>\n`;
    // Grid lines + Y labels
...
function pieArc(cx, cy, r, startAngle, endAngle, innerR = 0) {
...
function renderPieDonut(type, data, options, isDonut) {
