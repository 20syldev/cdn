...
function parseIPv4(raw) {
    ...
    const result = { ip: raw, version: 'IPv4', type, class: cls, binary, decimal, reverse };
    if (range)
        result.range = range;
    return result;
}
...
function parseIPv6(raw) {
    ...
    if (values.some((v) => isNaN(v) || v < 0 || v > 0xffff))
        throw new Error('Invalid IPv6 address');
    let type = 'public';
    let range;
    ...
    const result = { ip: expanded, version: 'IPv6', type, binary, reverse };
    if (range)
        result.range = range;
    return result;
}
 ...
 */
export default function ip(address) {
    if (!address || typeof address !== 'string')
        throw new Error('An IP address is required');
    if (address.length > MAX_STRING_LENGTH)
