...
function computeStrength(entropy) {
    if (entropy < 40)
        return 'weak';
    if (entropy < 60)
        return 'moderate';
 ...
 */
export default function password(type = 'random', length = 16, options = {}) {
    const { uppercase = true, lowercase = true, digits = true, symbols = false, exclude = '', count = 1, separator = '-', } = options;
