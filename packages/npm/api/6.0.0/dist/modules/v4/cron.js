...
function getComponents(date, formatter) {
 ...
 */
export default function cron(expr, count = 5, from, timezone = 'UTC') {
    if (!expr)
        throw new Error('Please provide a cron expression (?expr=* * * * *)');
    ...
    if (from && isNaN(fromMs))
        throw new Error('Please provide a valid from date (ISO 8601)');
