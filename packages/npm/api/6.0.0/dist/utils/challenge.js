...
const sign = (type, nonce, exp, payload) => createHmac('sha256', secret).update(`${type}.${nonce}.${exp}.${payload}`).digest('hex');
...
function prune(storage, now) {
    for (const [nonce, exp] of storage.usedNonces) {
        if (exp <= now)
            storage.usedNonces.delete(nonce);
    }
}
/**
 ...
 */
export function signToken(type, payload, extra = {}) {
 ...
 */
export function verifyToken(token, payload, storage) {
    ...
    if (storage.usedNonces.has(decoded.nonce))
