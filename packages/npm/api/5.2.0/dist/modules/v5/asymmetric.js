...
export default function asymmetric(action, options = {}) {
    ...
    if (action === 'encrypt') {
        ...
        try {
            const encrypted = publicEncrypt({ key: options.publicKey, oaepHash: algo.oaepHash, padding: cryptoConstants.RSA_PKCS1_OAEP_PADDING }, Buffer.from(options.text, 'utf8'));
