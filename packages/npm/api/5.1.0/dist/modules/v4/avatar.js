...
const VALID_TYPES = ['identicon', 'pixel'];
const VALID_FORMATS = ['png', 'svg'];
const MIN_SIZE = 50;
const MAX_SIZE = 2000;
...
function identicon(hash, size, bg, format) {
    const GRID = 5;
...
function pixel(hash, size, bg, format) {
    const GRID = 8;
    const cell = size / GRID;
    ...
    const isOn = (i) => ((hash[Math.floor(i / 8)] >> (7 - (i % 8))) & 1) === 1;
    const colorOf = (i) => palette[hash[Math.floor(i / 8)] % palette.length];
    if (format === 'svg') {
        const rects = [];
        for (let row = 0; row < GRID; row++) {
            for (let col = 0; col < GRID; col++) {
                ...
                if (isOn(i)) {
                    rects.push(`<rect x="${col * cell}" y="${row * cell}" width="${cell}" height="${cell}" fill="${colorOf(i)}" />`);
                }
            }
        }
        return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}"><rect width="${size}" height="${size}" fill="${bg}" />${rects.join('')}</svg>`;
...
export default function avatar(options = {}) {
    ...
    const type = (options.type ?? 'identicon');
